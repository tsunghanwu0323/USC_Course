...
export class SearchService {
  ...
  locationInput = this._locationInput.asObservable();
  ...
  search(form) {
    ...
    let params = new HttpParams()
      ...
      .set('geoJson', form.geoJSON.lat + ',' + form.geoJSON.lng);
    let response = this.http.get('http://localhost:3000/api/search', { params });
  ...
  getNextPage(pagetoken: string) {
    if (this.pageSaved.includes(pagetoken)) {
      ...
      let response = this.http.get('http://localhost:3000/api/nextPage', {params: params});
